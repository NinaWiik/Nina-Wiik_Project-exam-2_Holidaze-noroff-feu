...
function HotelDetail() {
    ...
    return (
        ...
        <Row  className="hotelDetail__row">
            ...
            <p><b>Max Guests:</b> {detail.maxGuests}</p>
            <p><b>Self Catering:</b> {detail.selfCatering ? 'Yes' : 'No'}</p>
            <p><b>Email:</b> <a href={"mailto:" + detail.email} className="hotelDetail__col--mail">{detail.email}</a></p>
            <p><b>Price:</b> {detail.price} $ per night</p>
            <Link to={`/accommodation/makeenquiry/${detail.id}`}> <Button className="hotelDetails__button--inquiry">Make Inquiry</Button></Link>
            </Col>
            <Col xs={8} lg={2}>
